...
# Init VALUES File
      ...
      export start_timestamp="20220101-000000"
...
do 
      ...
      echo "t_uid,mr_id,tstamp,anomalous,expected,max,min,value" > $WORKING_DIR_OUTPUT"/tararam.dt_metric_value-$actFile.csv"
      ...
      do
            ...
            echo "$LOOP_COUNT/$MAX_ITERATIONS - Creating for file "$actFile" - "$my_timestamp| sed 's/^/       /'
            ...
            do
                  ...
                  # Write line to temp file
                  echo $line >> $WORKING_DIR_OUTPUT"/tararam.dt_metric_value-$actFile.csv"
                  #echo $line 

            if [[ $ADD_SECONDS -ge 2677800 ]]; then
                  echo "        Done Seconds..."
                  break
            fi
