...
#
. ./tools/99_uninstall/3.6/uninstall-cp4waiops.props
...
check_namespaced_install () {
    ...
    if [[ ! -z "$operator_name" ]]; then
        ...
        log $INFO "\033[1;36mUninstalling AIOps from the namespace $CP4WAIOPS_PROJECT. IAF found in $IAF_PROJECT.\033[0m"
        return 0
    fi
    ...
    if [[ ! -z "$operator_name" ]]; then
        AIOPS_NAMESPACED="true"
        IAF_PROJECT=$CP4WAIOPS_PROJECT
        log $INFO "\033[1;36mUninstalling AIOps and IAF components from the namespace $CP4WAIOPS_PROJECT\033[0m"
        return 0
    else 
        ...
        log $INFO "\033[1;36mDefaulting to uninstalling resources from $CP4WAIOPS_PROJECT project.\033[0m"
        AIOPS_NAMESPACED="true"
        IAF_PROJECT=$CP4WAIOPS_PROJECT
    fi
}
...
delete_vault_catalog() {
    ...
    if [[ "$vault_catalog_exists" == "true" ]]; then
        oc delete catalogsource ibm-vault-operator-catalog -n openshift-marketplace
        log $INFO "ibm-vault-operator-catalog deleted"
    fi
}